...
use ai_env_cli::wire::argv::{self, Route};
...
/// untouched (the extension's env is the contract for a local child).
fn exec_local(bin: &Path, args: &[String]) -> ! {
...
fn main() {
    ...
    let inv = match argv::split(&argv) {
        Ok(i) => i,
        Err(e) => fail(2, &format!("{e}  (usage: ai-env-claude <realBinary> <claude args…> | --version)")),
    };
    // Kill switch: plain Cursor, no AWS, no runtime.
